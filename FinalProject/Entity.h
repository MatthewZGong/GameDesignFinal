...
struct AttackInfo{
    ...
    AttackInfo(int d, float k): dmg(d), knock_back(k){
...
public:
    ...
    virtual void draw_sprite_from_texture_atlas(ShaderProgram* program, GLuint texture_id, int index);
    virtual void update(float delta_time, Entity* collidable_entities, int collidable_entity_count, Map* map);
    
    virtual void update(float delta_time, Entity* main_spawn, std::vector<Entity*>& collidable_entities, Map* map);
    virtual void render(ShaderProgram* program);
    
    
    virtual void update_effects(float delta_time, Entity* main_spawn, std::vector<Entity*>& collidable_entities, Map* map);
    virtual void render_effects(ShaderProgram* program);

    bool const check_collision(Entity* other) const;
    void const check_collision_y(Entity* collidable_entities, int collidable_entity_count);
    void const check_collision_x(Entity* collidable_entities, int collidable_entity_count);
    void const check_collision_y(std::vector<Entity*>& collidable_entities);
    void const check_collision_x(std::vector<Entity*>& collidable_entities);
...
//     Overloading our methods to check for only the map
    ...
    std::pair<glm::vec3,glm::vec3> get_x_boundary();
